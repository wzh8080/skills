...
import java.lang.*;
/*
 * 这个程序的实现用到了编码器LAME.exe这个的作用是将录音得到的wav文件编码成MP3文件，
 * 这样的结果是容量大大减少，因为LAME编码器经测试在编码过程中路径中带有空格问题未能解决，
 * 所以只能找了一个笨的方法，为了将其放到指定的文件夹下(这个文件夹路径是带有空格的)，所以只能先存放到D盘，
 * 然后利用文件操作将其复制到指定的路径下，然后再删除
 * 原文链接：https://blog.csdn.net/computer_liuyun/article/details/17468555
 */
/**
 * 实现录音机的功能
